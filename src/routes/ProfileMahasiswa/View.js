...
function ProfileMahasiswa() {
  ...
  const Mahasiswa = () => {
    ...
    if(!loading && typeof mahasiswa !== 'object') return 'Data tidak ditemukan';

    return (
      <div>
        <h2>Profile Mahasiswa #{id}</h2>
        <ProfilePicture width={100} height={100} imageUrl={mahasiswa.photo} />
        <p><b>{mahasiswa.name} ({mahasiswa.nim})</b></p>
        <p>{mahasiswa.degree} - {mahasiswa.major_name} - {mahasiswa.faculty_name}</p>
        <p>Alamat: {mahasiswa.address.street}, {mahasiswa.address.city}</p>
      </div>
    )
  }
